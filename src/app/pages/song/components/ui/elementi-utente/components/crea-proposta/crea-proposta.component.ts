...
export class CreaPropostaComponent {
  ...
  public changePicForm: RecordStruttura = {
    ...
    file: {
      ...
      onChange: (x: File) => this.updateLinkByFile(x),
      file: {
        previewUrl: null,
  ...
  private uploadFileHttp(p: ProposaPrePost): void {
    ...
    handlerFunc({
      callHttp: () =>
        forkJoin({
          main: this.dropboxService.uploadFile(p.file, this.userId, p.basePath),

          copertina: this.dropboxService.uploadFile(
            p.copertina,
            this.userId,
            'Copertine' + this.tipoProposta,
          ),
        }),
      ...
      nextCall: () => {
        ...
        this.elementiUtenteService.utenteParodieLoaded = false;
      },
    });
  }

  private updateLinkByFile(file: File): void {
    switch (file.type) {
      case 'application/pdf':
        this.tipoProposta = 'Manga';
        break;
      case 'audio/mpeg':
        this.tipoProposta = 'Canzone';
        break;
    }
  }
}
